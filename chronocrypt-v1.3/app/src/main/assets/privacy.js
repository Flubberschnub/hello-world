...
function sync(){
 ...
 if(aiThinking){
  if(overlay.dataset.aiBlock!=='true'){
   ...
   overlay.innerHTML='<div class="modal hero"><h2>CONTINUUM AI</h2><p>Calculating causal lines, prepared reactions, and combat commands…</p></div>';
  }
 }else if(overlay.dataset.aiBlock==='true'){
  delete overlay.dataset.aiBlock;overlay.classList.remove('show');overlay.innerHTML='';
 }
}
